...
function update_config($configPath, $platform) {
	...
	if ($configXml -ne $null) {
		...
		if ($platform -ne $null -and $platform -ne 'WebApi') {
			$validateIntegratedMode = $configXml.SelectSingleNode("configuration/system.webServer/validation")
			if (($validateIntegratedMode -eq $null) -or (($validateIntegratedMode.HasAttribute("validateIntegratedModeConfiguration") -and [System.Convert]::ToBoolean($validateIntegratedMode.GetAttribute("validateIntegratedModeConfiguration"))) -eq $false)) {
				$webModule = $configXml.SelectSingleNode("configuration/system.web/httpModules/add[@name='ExceptionlessModule']")
				if ($webModule -eq $null) {
					$parentNode = $configXml.SelectSingleNode("configuration/system.web")
					if ($parentNode -eq $null) {
						$parentNode = $configXml.SelectSingleNode("configuration").AppendChild($configXml.CreateElement('system.web'))
					}
					$parentNode = $configXml.SelectSingleNode("configuration/system.web/httpModules")
					if ($parentNode -eq $null) {
						$parentNode = $configXml.configuration['system.web'].AppendChild($configXml.CreateElement('httpModules'))
					}
					$webModule = $configXml.CreateElement('add')
					$webModule.SetAttribute('name', 'ExceptionlessModule')
					$webModule.SetAttribute('type', 'Exceptionless.' + $platform + '.ExceptionlessModule, Exceptionless.' + $platform)
				
					$parentNode.AppendChild($webModule)
					$shouldSave = $true
				}
			}
